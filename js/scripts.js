        ...
        <div class="flip-card flip-back flip-${cardType}${mode == 'init' ? ` startingPos` : ` expanded`}">
            <div class="flip-card-inner">
                ...
                <div class="flip-card-back">
                    <div class="cardContainer expanded" type="${cardType}"${cardType == 'plant' ? ` lighting="${thisCard.lighting.length}"` : ``}>
                        <img class="${cardType} expanded" src="img/${cardType}s/${thisCard.img}.jpg" alt="" />
                        ${cardType == 'plant' ? `
...
function initMarketFunc(){
    ...
    // let currentItem = document.querySelector(`.marketColumn[column="${currentColumn}"] ${marketItemClasses[currentMarketItem]}`);
...
function initPlayersHome() {
    ...
    // category:  market
    // elem:  #marketSection .inactive
    // mode:  collapsed

    setTimeout(function(){
        positionAnimation('tableau', $('#playerInfoContainer #cardToPlace .flip-plant'), 'tableauStartingPos');
        positionAnimation('tableau', $('#mapContainer #mapHiddenOverlay #row-2-column-4 .flip-room'), 'tableauStartingPos');
    }, 1000);
